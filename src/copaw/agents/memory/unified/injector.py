...
class DynamicInjector:
    ...
    def _format_entities(self, entities: list[Entity], max_tokens: int) -> str:
        ...
        **Safety entities (priority >= 100) are ALWAYS injected, never truncated.**
        Token budget only applies to important and normal entities.
        
        Args:
            entities: Entities to format
            max_tokens: Token budget (only for non-safety entities)
        ...
        # ============================================================
        # SAFETY SECTION: ALWAYS INJECTED, NO TOKEN LIMIT
        # Safety entities are critical (allergies, constraints) and must
        # never be truncated regardless of token budget.
        # ============================================================
        if safety:
            header = "### ⚠️ 安全相关（必须注意）"
            lines.append(header)
            ...
            # Log safety token consumption (for monitoring, not limiting)
            logger.debug(f"Safety entities injected: {len(safety)} items, {estimated_tokens} tokens")
            
            # Warn if safety section exceeds budget (but still inject)
            if estimated_tokens > max_tokens:
                logger.warning(
                    f"Safety section ({estimated_tokens} tokens) exceeds budget ({max_tokens}), "
                    "but still injected (safety never truncated). "
                    "Other sections may be reduced."
                )
        
        # ============================================================
        # IMPORTANT & NORMAL SECTIONS: SUBJECT TO TOKEN BUDGET
        # Use remaining budget after safety section
        # ============================================================
        remaining_budget = max_tokens - estimated_tokens
        
        # Important section
        if important and remaining_budget > 50:  # Need at least 50 tokens for header + some content
            lines.append("")  # Blank line
            header = "### 重要信息"
            lines.append(header)
            remaining_budget -= len(header) // self.CHARS_PER_TOKEN + 1
            ...
            for entity in important:
                ...
                if remaining_budget < token_cost:
                    logger.debug(f"Important section truncated: {remaining_budget} tokens remaining")
                    break
        ...
        # Normal section
        if normal and remaining_budget > 50:
            lines.append("")  # Blank line
            header = "### 其他信息"
            lines.append(header)
            remaining_budget -= len(header) // self.CHARS_PER_TOKEN + 1
            ...
            for entity in normal:
                ...
                if remaining_budget < token_cost:
                    logger.debug(f"Normal section truncated: {remaining_budget} tokens remaining")
                    break
                
                lines.append(text)
                remaining_budget -= token_cost
        
        total_tokens = max_tokens - remaining_budget
        logger.info(f"Injected {len(safety)} safety + {len(lines)-len(safety)-3} other entities, ~{total_tokens} tokens")
    ...
    def _format_single_entity(self, entity: Entity, is_safety: bool = False) -> str:
        ...
        if entity.name and entity.name != entity.content:
            ...
            return f"{prefix}{entity.content}{suffix}"
